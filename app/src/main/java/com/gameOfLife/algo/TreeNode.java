...
public class TreeNode extends TreeNodeBase {
    ...
    TreeNode nextGeneration() {
        ...
        TreeNode node=create(create(n00, n01, n10, n11).nextGeneration(),
    ...
    int[][] getBitArray(int gridSize){//if grid size is say 20 X 20 then pass 20
        long t=System.currentTimeMillis();
        int[][] grid=new int[gridSize][gridSize];
        TreeNode innerNode;
        int neededLevel=1;//tree height
        int temp=2;
        while(temp<gridSize){
            temp*=2; neededLevel++;
        }
        Log.d("111","getBitArray: "+neededLevel+", "+this.level);
        if(this.level<=neededLevel){
            innerNode=this;
        }else{
            int currentLevel=this.level;
            innerNode=this.centeredSubnode();
            while(currentLevel>neededLevel){
                innerNode=innerNode.centeredSubnode();
                --currentLevel;
            }
        }
        Log.d("111"," Time taken to get the innerNode of right grid:"+(System.currentTimeMillis()-t));
        int k=gridSize/2;
        for(int j=-k+1;j<k+1;j++) {
            for (int i = -k+1; i < k+1; i++) {
                int p=innerNode.getBit(i,j);
                grid[j+k-1][i+k-1]=p;
            }
        }

         Log.d("111"," Time taken to get the right grid:"+(System.currentTimeMillis()-t));
        return grid;
    }
